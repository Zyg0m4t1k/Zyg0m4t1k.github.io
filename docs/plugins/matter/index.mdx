...
| Bouton | Description |
|--------|-------------|
| **Infos avancées du nœud** | Affiche une vue structurée des clusters, attributs et endpoints du nœud Matter |
| **Dump node brut** | Récupère et affiche la structure JSON complète du nœud Matter (clusters, attributs, endpoints) |

La modal **Infos avancées** présente les données de façon lisible par cluster et endpoint. Le **Dump node brut** affiche le JSON complet avec un bouton **Copier** pour faciliter le partage lors d'un ticket de support.

:::tip Firmware
La version firmware et les éventuelles mises à jour OTA disponibles sont visibles dans la **Page Santé** (tableau centralisé de tous les équipements).
:::
...
| `is_locked` | Info binaire | Verrouillée (1) / Déverrouillée (0) |
| `lock_state_text` | Info texte | État lisible (Verrouillé / Déverrouillé / Bloqué) |
| `lock_state_raw` | Info numérique | Valeur brute Matter (0/1/2/3) |
| `set_lock_mode` | Action | Mode de la serrure |
| `lock_last_action` | Info | Dernière action (Verrouillage, Déverrouillage…) |
| `lock_last_source` | Info | Source (Manuel, Clavier, Distant, Biométrique…) |
| `lock_last_credential` | Info | Type de credential (Code PIN, RFID, Empreinte…) |
| `lock_last_date` | Info | Horodatage de la dernière action |
...
Créées automatiquement si l'appareil le supporte.

| Commande | Description |
|----------|-------------|
| `ota_update_state` | État de la mise à jour (valeur numérique, voir tableau ci-dessous) |
| `ota_update_progress` | Progression en % (-1 si non applicable) |

| Valeur | Signification |
|--------|---------------|
| 0 | Unknown |
| 1 | Idle — pas de mise à jour en cours |
| 2 | Querying — interrogation du serveur OTA |
| 3 | DelayedOnQuery — nouvel essai différé |
| 4 | Downloading — téléchargement en cours |
| 5 | Applying — installation en cours |
| 6 | DelayedOnApply — application différée |
| 7 | RollingBack — retour arrière |
| 8 | DelayedOnUserConsent — attente confirmation utilisateur |

---

## ❤️ Page Santé

La page Santé (bouton **Santé** sur la page principale du plugin) affiche un **tableau centralisé de tous vos équipements Matter** avec leur état en temps réel.

{/* ![Page santé — tableau équipements](/img/plugins/matter/health_devices.png) */}

### Colonnes du tableau

| Colonne | Description |
|---------|-------------|
| **ID** | Numéro de nœud Matter |
| **Nom** | Nom de l'équipement Jeedom |
| **Actif** | L'équipement est activé dans Jeedom |
| **Visible** | L'équipement est visible dans Jeedom |
| **Disponible** | 🟢 Joignable / 🔴 Non joignable (temps réel) |
| **Produit** | Fabricant et nom du produit |
| **Firmware actuel** | Version installée sur l'appareil |
| **MAJ disponible** | Version disponible via OTA (badge jaune si mise à jour) |
| **Dernière vérif.** | Date de la dernière vérification firmware |
| **Types Matter** | Types de device Matter détectés |

### Actions disponibles

- **Actualiser** : recharge le tableau
- **Vérifier tous les firmware** : lance une vérification OTA sur tous les équipements
- **Mettre à jour** *(bouton par ligne)* : déclenche la mise à jour firmware directement depuis le tableau, sur les équipements pour lesquels une version plus récente est disponible (badge jaune)

:::tip
L'indicateur **Disponible** (🟢/🔴) reflète la valeur de la commande `available` de chaque équipement, mise à jour en temps réel par le démon.
:::

### Indicateur de disponibilité sur la page principale

Un point vert ou rouge apparaît également sur chaque carte d'équipement de la page principale du plugin, permettant de voir en un coup d'œil quels appareils sont joignables.

{/* ![Indicateur disponible carte équipement](/img/plugins/matter/health_available_card.png) */}

---

## 🔍 Page Diagnostic

La page Diagnostic (bouton **Diagnostic** sur la page principale) affiche un **rapport de vérification complet** de l'état du plugin.

{/* ![Page diagnostic](/img/plugins/matter/diagnostic_page.png) */}
...
- **Diagnostiquer** : rafraîchit le rapport
