...
- **Mini-Cal** : affiche le widget sous forme de grille mensuelle (au lieu de la liste FullCalendar)
...
L'option **Mini-Cal** (disponible dans les configurations url, local, caldav et widget) remplace le widget liste FullCalendar par une **grille mensuelle** :

- Affichage du mois en cours avec navigation précédent/suivant
- Chaque jour indique la présence d'événements : point vert et compteur si des événements existent, point rouge sinon
- Les jours du mois précédent et suivant apparaissent en opacité réduite
- Un **clic sur un jour** ouvre une tuile flottante listant les événements du jour (heure ou "Toute la journée" et titre)
- Un bouton **"Liste"** dans la tuile permet de basculer vers la vue liste FullCalendar, avec un bouton **"Mois"** pour revenir à la grille

![Widget Mini-Cal — vue grille](/img/plugins/ics/mini_cal_grid.png)
![Widget Mini-Cal — tuile jour](/img/plugins/ics/mini_cal_tile.png)

---

### Widget Carte Événement

Les équipements de type **Événement** disposent d'un widget carte dédié qui affiche le prochain événement correspondant.

![Widget carte événement — aucun événement](/img/plugins/ics/eventcal_empty.png)
![Widget carte événement — avec données](/img/plugins/ics/eventcal_widget.png)

Le widget affiche :
- le **mois** dans l'entête colorée
- le **numéro du jour** en grand
- le **jour de la semaine**
- l'**heure** (masquée si journée entière)
- le **titre** de l'événement
- la **localisation** si renseignée

L'entête change de couleur selon l'imminence de l'événement :

| Délai | Couleur |
|---|---|
| Plus de 24h | Vert |
| Moins de 24h | Orange |
| Moins d'1h | Rouge vif |
| Moins de 5 min | Rouge foncé |
| Terminé | Gris |
| Aucun événement | Gris + "Aucun événement" |

### Thèmes Light et Dark

Le widget calendrier et le widget carte événement s'adaptent automatiquement au thème Jeedom actif (Light ou Dark).

<!-- ![Widget calendrier thème Light](/img/plugins/ics/theme_light.png) -->
<!-- ![Widget calendrier thème Dark](/img/plugins/ics/theme_dark.png) -->
