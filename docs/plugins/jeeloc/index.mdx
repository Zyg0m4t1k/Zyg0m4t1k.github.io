...
| Service | Gratuit | Quota gratuit | Clé API requise |
|---|---|---|---|
| **Nominatim** *(OpenStreetMap)* | Oui | 1 requête/seconde max sur le serveur public OSM *(pas de quota mensuel fixe)* | Non |
| **MapQuest** Open Geocoding | Oui | 15 000 transactions/mois | Oui |
| **Mapbox** Geocoding | Oui | 100 000 requêtes/mois | Oui |

| Service | Avantages | Inconvénients |
|---|---|---|
| **Nominatim** | Gratuit, sans inscription, open source | Limité à 1 req/s, politique d'usage stricte |
| **MapQuest** | Gratuit jusqu'à 15 000 req/mois, basé sur OSM | Nécessite une clé API |
| **Mapbox** | Très bonne qualité, 100 000 req/mois gratuites | Nécessite une clé API |
...
| Durée | Info numérique | min | Durée estimée du trajet en minutes (avec trafic) |
| Durée habituelle | Info numérique | min | Durée sans trafic (référence Mapbox). Disponible uniquement avec le profil **Voiture (avec trafic)** |
...
| Mode | Info texte | — | Libellé du profil utilisé (ex : `Voiture (trafic)`) |

### 🖥️ Widget Trajet

Le widget Trajet affiche dans l'ordre :

- l'icône du mode de transport et le nom du trajet dans l'en-tête, avec un bouton de recalcul manuel
- la **durée en grand** (ex : `23 min` ou `1h05`) accompagnée du **delta de trafic** (ex : `+9 min` en orange si retard, vert si gain)
- l'**arrivée estimée** calculée depuis l'heure du dernier calcul + la durée (ex : `Arrivée estimée à 08:42`)
- une **barre colorée** indiquant le niveau de trafic global : verte (fluide), jaune (modéré), orange (dense), rouge (très dense)
- un tableau synthétique : **Distance** et **Trafic** (masqué si fluide)
- le temps écoulé depuis le dernier calcul (ex : `Mis à jour il y a 2 min`)

Si aucun calcul n'a encore été effectué, le widget affiche une icône d'itinéraire vide.

:::info
Le delta de trafic et la barre colorée ne sont significatifs qu'avec le profil **Voiture (avec trafic)**, qui est le seul à fournir une durée habituelle de référence.
Pour les autres profils, la durée habituelle est identique à la durée calculée et aucun delta n'est affiché.
:::
