...
- Description météo
- Code météo WMO
- Code météo WMO H / H+1 / H+2

### 🌤️ Codes météo WMO

Le **code météo WMO** est un nombre standard utilisé par les services météo pour décrire les conditions actuelles ou prévues.

| Code(s) | Conditions |
|---------|------------|
| 0 | Ciel dégagé |
| 1, 2, 3 | Partiellement nuageux, nuageux, couvert |
| 45, 48 | Brouillard / Dépôt de givre |
| 51, 53, 55 | Bruine légère, modérée, dense |
| 56, 57 | Bruine verglaçante légère, dense |
| 61, 63, 65 | Pluie faible, modérée, forte |
| 66, 67 | Pluie verglaçante légère, forte |
| 71, 73, 75 | Neige faible, modérée, forte |
| 77 | Grains de neige |
| 80, 81, 82 | Averses de pluie faibles, modérées, violentes |
| 85, 86 | Averses de neige faibles, fortes |
| 95 | Orage faible ou modéré |
| 96, 99 | Orage avec grêle faible, forte |

Les commandes **H**, **H+1** et **H+2** donnent le code WMO pour l'heure en cours et les deux heures suivantes.

### ☀️ Rayonnement solaire

Le plugin fournit le **rayonnement solaire à courte onde** (en W/m²), qui indique la puissance du rayonnement solaire atteignant la surface à un instant donné.

Trois commandes sont disponibles :
- **Rayonnement solaire** — heure courante
- **Rayonnement solaire H+1** — heure suivante
- **Rayonnement solaire H+2** — dans deux heures

Ces données proviennent des données horaires Open-Meteo (`shortwave_radiation`).
...
- la pression
- le vent (vitesse + rafales) avec **boussole SVG** indiquant la direction
- les précipitations
...
Il affiche par défaut les **prochaines 24h** depuis l’heure actuelle (bouton `Auj.`), ou une journée future au choix (`J+1` à `J+4`).

Contenu du graphique :
- courbes température, précipitations, humidité, vitesse du vent
- **icônes météo officielles** toutes les 2h avec label heure et tooltip de la condition
- **boussoles SVG** pour la direction du vent, avec tooltip vitesse + cardinal (ex: `25 km/h - SSO`)
- **bouton toggle** `fa-layer-group` pour masquer/afficher les barres icônes et vent — le graphique s’agrandit automatiquement avec une animation fade
