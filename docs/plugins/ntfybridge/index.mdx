...
- Nécessite un reverse proxy (Apache, Nginx, NPM, Traefik, Cloudflare…)
...
```text
Plugins → Gestion des plugins → Market → ntfybridge → Installer stable
```
...
## 2 — Configurer le reverse proxy (mode embarqué + sous-domaine)
...
Groupez vos paramètres récurrents en profils nommés.

Ouvrir depuis **Plugins → ntfybridge → Profils** :
...
```text
[profile=urgent] Intrusion détectée
[profile=discret] Rapport journalier prêt
[profile=urgent][priority=3] Priorité surchargée
```
...
| **Utilisateurs autorisés** | Liste des utilisateurs ntfy autorisés à envoyer des commandes secours (séparés par virgule). Vide = tous les utilisateurs. |
...
| `log ntfybridge` | Les 20 dernières lignes du log ntfybridge |
...
- L'utilisateur doit figurer dans **Utilisateurs autorisés** pour que ses commandes soient traitées
:::
...
1. App **Raccourcis** → nouveau raccourci → **Obtenir le contenu de l'URL**
2. URL : `https://ntfy.votredomaine.fr/mon_topic` · Méthode : `POST`
3. Corps : le message à envoyer (ex: `éteins tout`)
4. Ajouter en widget écran d'accueil ou déclencher via **Siri**
