...
| 3 | **Configurer le serveur ntfy** | Mode embarqué : définir l'identifiant admin et le mot de passe → [voir détails ↓](#configurer-le-serveur-ntfy) |
...
| 6 | **Tester le reverse proxy** | Lancer l'assistant intégré, suivre les 5 étapes → [voir reverse proxy ↓](#reverse-proxy) |
| 7 | **Créer un utilisateur ntfy** | Panel utilisateurs : ajouter un compte avec droits sur le topic secours → [voir panel ↓](#gérer-les-utilisateurs-panel) |
| 8 | **Installer l'app ntfy** | Télécharger sur [iOS](https://apps.apple.com/app/ntfy/id1625396347) / [Android](https://play.google.com/store/apps/details?id=io.heckel.ntfy), ajouter le serveur, s'authentifier → [voir app ↓](#installer-et-configurer-lapp-ntfy) |
| 9 | **Configurer les topics secours et tester le ping** | Modale Secours : renseigner le topic action et réponse, activer `ping`, envoyer `ping` depuis l'app → [voir secours ↓](#mode-secours) |
...
## 🖥️ Configurer le serveur ntfy {#configurer-le-serveur-ntfy}
...
<TabItem value="embedded" label="⭐ Mode embarqué (recommandé)" default>
...
:::tip Première mise en route
1. Sauvegarder la configuration → le port est auto-détecté
2. Démarrer le démon → le serveur ntfy démarre automatiquement
3. (Optionnel) Lancer l'assistant reverse proxy pour l'accès externe
:::

</TabItem>
<TabItem value="external" label="Mode externe">
...
## 👥 Gérer les utilisateurs (panel) {#gérer-les-utilisateurs-panel}

```text
Plugins → ntfybridge → Panel
```

:::info Mode embarqué uniquement
...
2. Saisir un nom d'utilisateur et un mot de passe (minimum 8 caractères)
3. Cliquer sur **Créer**
...
## 📱 Installer et configurer l'app ntfy {#installer-et-configurer-lapp-ntfy}
...
| **Android** | [Google Play](https://play.google.com/store/apps/details?id=io.heckel.ntfy) ou [F-Droid](https://f-droid.org/packages/io.heckel.ntfy/) |
| **Web** | Accéder à l'URL de votre serveur ntfy dans un navigateur |

#### S'abonner à un topic

1. Ouvrir l'app ntfy
2. Appuyer sur **+** (ajouter un abonnement)
3. Configurer selon votre mode :

<Tabs groupId="server-setup">
<TabItem value="embedded" label="⭐ Mode embarqué" default>

- **Avec sous-domaine** : saisir l'URL complète `https://ntfy.votredomaine.fr/votre_topic`
- **En local / VPN** : saisir `http://IP_JEEDOM:PORT_NTFY/votre_topic`
- Si authentification : renseigner le nom d'utilisateur et le mot de passe créés dans le [panel](#gérer-les-utilisateurs-panel)

</TabItem>
<TabItem value="external" label="Mode externe">

- **ntfy.sh** : saisir uniquement le nom du topic (ex: `maison_cmd`)
- **Auto-hébergé** : saisir l'URL complète `https://ntfy.monserveur.fr/votre_topic`
- Si authentification : renseigner vos identifiants ntfy
...
|---|---|
| `ping` | Confirmation de vie + heure du serveur, charge CPU, RAM libre, uptime, résolution DNS Jeedom |
...
| **Script** | Fichier à exécuter, sélectionné parmi les scripts disponibles dans `data/rescue_scripts/`. L'interpréteur est déduit automatiquement de l'extension (`.sh`→bash, `.py`→python3, `.php`→php, `.js`→node) |
...
## 🌐 Configurer le reverse proxy (mode embarqué + sous-domaine) {#reverse-proxy}
...
- Vérifier Node.js ≥ 22 : `node --version`
